...
function medikament(id: string) {
  ...
  if (!eintrag) throw new Error(`Medikament «${id}» fehlt im Pilotkatalog`)
  return eintrag
}
...
describe('Medikamenten-Pilotkatalog', () => {
  it('bleibt bis zur UI-Freigabe ausdrücklich ein Pilot mit zehn Wirkstoffen', () => {
    expect(medikamentKatalog.status).toBe('pilot')
    expect(medikamentKatalog.medikamente).toHaveLength(10)
...
describe('Sicherheitsgrenzen der Medikamentenengine', () => {
  ...
  it('führt verschreibungspflichtige Pilotfälle nicht als Selbstmedikation', () => {
